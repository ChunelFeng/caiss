...
#include "../../../utilsCtrl/lruProc/LruProc.h"
...
int mainFunc() {
    ...
    SetConsoleTitle("CAISS-DEMO");
    int ret = 0;
    if (ACTION == 0) {
        ret = hnsw_train();
    } else if (ACTION == 1) {
        ret = hnsw_search(SEARCH_WORD);
    } else if (ACTION == 2) {
...
int LruFunc() {
    auto lru = new LruProc();
    for(int i = 0; i < 10; i++) {
        lru->put(std::to_string(i), std::to_string(i*i));
    }
...
int lruTest() {
    ...
    vector<string> vec = {"hello", "world", "test", "hello", "fly", "see", "say", "last", "hello"};
    for (auto a : vec) {
        ret = proc->search((void *)a.c_str(), SEARCH_TYPE, TOP_K);
        CAISS_FUNCTION_CHECK_STATUS
    }
